...
func TestAlertRuleSurvivesRestart(t *testing.T) {
	dir := t.TempDir()
	...
	newRouter := func() *gin.Engine {
		cfg := &config.Config{Port: 8080, CollectInterval: 2 * time.Second, RetentionWindow: time.Hour, LogLevel: "error", Version: "test"}
		col := collector.New(2*time.Second, time.Hour, "", log)
		store, err := alert.NewStore(path, log)
		if err != nil {
			t.Fatal(err)
		}
		return NewRouter(cfg, log, col, &dockermon.Monitor{}, store)
	}

	valid := `{"name":"重启后保留","metric":"cpu_percent","op":">","threshold":80,"duration_sec":30,"cooldown_sec":300,"enabled":true,"webhook_url":"http://localhost:8080/api/mock/webhook"}`
	w, body := doJSON(t, newRouter(), http.MethodPost, "/api/alert-rules", valid)
	if w.Code != http.StatusCreated {
		t.Fatalf("创建应 201，实际 %d: %v", w.Code, body)
	}
	createdID := body["data"].(map[string]any)["id"].(string)

	w, body = doJSON(t, newRouter(), http.MethodGet, "/api/alert-rules", "")
	if w.Code != http.StatusOK {
		t.Fatalf("重启后列表应 200，实际 %d: %v", w.Code, body)
	}
	rules := body["data"].([]any)
	if len(rules) != 1 || rules[0].(map[string]any)["id"] != createdID {
		t.Fatalf("重启后应读回刚创建的规则 %q，实际 %v", createdID, rules)
	}
}
